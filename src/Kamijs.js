...
import { open } from "./core/SqliteAdapter.js";
...
export class Kamijs {
    ...
    async init() {
        ...
        this.db = await open({ filename: this.dbPath });
        await this.db.exec(`
    ...
    async cleanInactiveUsers() {
        ...
        const result = await this.#transaction(async () => {
            ...
            await this.db.run(`
                DELETE FROM market WHERE seller_jid IN (
                    SELECT jid FROM users WHERE last_active > 0 AND last_active < ?
                )
            `, [cutoff]);
            await this.db.run(`
                DELETE FROM claims WHERE owner_jid IN (
                    SELECT jid FROM users WHERE last_active > 0 AND last_active < ?
                )
            `, [cutoff]);
