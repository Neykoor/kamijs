...
export class Kamijs {
    ...
    async useTicket(jid, charId, sock) {
        ...
        try {
            ...
            if (!char) throw new Error("CHARACTER_NOT_FOUND");
            if (char.global_limit && (await this.db.get("SELECT COUNT(*) as count FROM claims WHERE char_id = ?", [char.id])).count >= char.global_limit) throw new Error("OUT_OF_STOCK");
            if (await this.db.get("SELECT * FROM claims WHERE char_id = ? AND owner_jid = ?", [char.id, userJid])) throw new Error("ALREADY_OWNS");
    ...
    async buyFromMarket(jid, marketId, sock) {
        ...
        try {
            ...
            if (!await this.db.get("SELECT * FROM claims WHERE char_id = ? AND owner_jid = ?", [listing.char_id, listing.seller_jid])) {
                await this.db.run("DELETE FROM market WHERE id = ?", [marketId]);
                throw new Error("SELLER_NO_LONGER_OWNS");
            }
            if (await this.db.get("SELECT * FROM claims WHERE char_id = ? AND owner_jid = ?", [listing.char_id, userJid])) throw new Error("ALREADY_OWNS");
    ...
    async addCharacter(data) {
        ...
        if (await this.db.get("SELECT id FROM characters WHERE LOWER(name) = LOWER(?) AND LOWER(series) = LOWER(?)", [data.name, data.series])) throw new Error("DUPLICATE_CHARACTER");
    ...
    async getRandomCharacterBySeries(series) {
        ...
        return await this.db.get("SELECT * FROM characters WHERE id = ?", [chars[Math.floor(Math.random() * chars.length)].id]);
    }
    ...
    async withdrawBank(amount, toJid, sock) {
        ...
        await this.db.run("BEGIN IMMEDIATE");
        try {
            if (await this.getBank() < amount) throw new Error("BANK_INSUFFICIENT_FUNDS");
    ...
    async trade(fromJid, toJid, charId, sock) {
        ...
        await this.db.run("BEGIN IMMEDIATE");
        try {
            if (!await this.db.get("SELECT * FROM claims WHERE char_id = ? AND owner_jid = ?", [charId, from])) throw new Error("CHARACTER_NOT_OWNED");
        ...
        try {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                if ((p >= PITY_LIMIT_RW && !hitOccurred) || Math.random() < effectiveRate) {
                    ...
                    if (await this.db.get("SELECT owner_jid FROM claims WHERE char_id = ? AND owner_jid = ?", [char.id, userJid]) || pulledThisSession.has(char.id)) {
                        isRepeat = true;
            ...
            for (const claim of newClaims) await this.db.run("INSERT INTO claims (char_id, owner_jid, claimed_at) VALUES (?, ?, ?)", [claim.char_id, claim.owner_jid, claim.claimed_at]);
    ...
    async _getRandom(excludeSet = new Set()) {
        ...
        const candidates = await this.db.all(`SELECT c.id FROM characters c WHERE ${conditions.join(" AND ")}`, params);
    ...
    async delistMarket(jid, marketId, sock) {
        ...
        await this.db.run("BEGIN IMMEDIATE");
        try {
            const listing = await this.db.get("SELECT * FROM market WHERE id = ? AND seller_jid = ?", [marketId, userJid]);
            if (!listing) throw new Error("LISTING_NOT_FOUND");
            await this.db.run("DELETE FROM market WHERE id = ?", [marketId]);
            await this.db.run("COMMIT");
        } catch (e) {
    ...
    async releaseCharacter(jid, charId, sock) {
        ...
        await this.db.run("BEGIN IMMEDIATE");
        try {
            if (!await this.db.get("SELECT * FROM claims WHERE char_id = ? AND owner_jid = ?", [charId, userJid]))
