        ...
        try {
            for (let i = 0; i < 10; i++) {
                ...
                if (Math.random() < 0.02) {
                    droppedTicket = true;
                    await this.db.run(`UPDATE users SET tickets = tickets + 1 WHERE jid = ?`, [userJid]);
                    // Notificar al usuario por DM que obtuvo un ticket
                    if (sock) {
                        ...
                        const notifMsg = [
                            ...
                            `━━━━━━━━━━━━━━━━━━━━`,
                            `¡Felicidades! Conseguiste un Ticket de Selección durante tu tirada.`,
                            ``,
                            `📦 *Tickets actuales:* ${totalTickets}`,
                            ``,
                            `_Úsalo con .ticket [Nombre o ID] para intentar capturar al personaje que quieras._`
                        ].join('\n');
                        sock.sendMessage(userJid, { text: notifMsg }).catch(() => {});
                    }
                }
                results.push({ ...(char || {}), isRepeat, repeatCompensation, jackpotBonus, droppedTicket, pity: p, luck: parseFloat(luck.toFixed(4)) });
            }
            if (bankAccrued > 0) await this.db.run('UPDATE bank SET balance = balance + ? WHERE id = 1', [bankAccrued]);
            const result = await this.db.run(`UPDATE users SET balance = balance - ?, pity_count = ?, luck = ? WHERE jid = ? AND balance >= ?`, [PULL_COST, p, luck, userJid, PULL_COST]);
