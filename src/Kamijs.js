...
export class Kamijs {
    ...
    async cleanInactiveUsers() {
        ...
        try {
            // Una sola subquery para market y claims, luego borra users
            await this.db.run(`
                DELETE FROM market WHERE seller_jid IN (SELECT jid FROM users WHERE last_active > 0 AND last_active < ?);
                DELETE FROM claims WHERE owner_jid  IN (SELECT jid FROM users WHERE last_active > 0 AND last_active < ?);
            `, [cutoff, cutoff]);
    ...
    async pull10(jid, options = {}) {
        ...
        // Evento opcional: costo distinto, multiplicador de rate, y garantia de
        // un minimo de hits en la tirada. Si no se pasa eventConfig, el
        // comportamiento es identico al original (PULL_COST / HIT_RATE_RW x1).
        ...
        try {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                // Garantia de evento: si quedan exactamente tantos slots como
                // hits aun le faltan al minimo garantizado, este slot se
                // fuerza a impactar (ej. guaranteedMin=1 y vamos en 0 hits con
                // 1 slot restante -> se fuerza el ultimo tiro).
            ...
            if (newClaims.length > 0) {
                // Un único INSERT para todas las claims nuevas de la tirada
                const stmt = await this.db.prepare("INSERT OR IGNORE INTO claims (char_id, owner_jid, claimed_at) VALUES (?, ?, ?)");
                for (const c of newClaims) await stmt.run(c.char_id, c.owner_jid, c.claimed_at);
                await stmt.finalize();
            }
            if (jackpotTotal > 0)
