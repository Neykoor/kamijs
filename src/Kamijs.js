...
export class Kamijs {
    ...
    async cleanInactiveUsers() {
        ...
        const result = await this.#transaction(async () => {
            ...
            const { changes } = await this.db.run("DELETE FROM users WHERE last_active > 0 AND last_active < ?", [cutoff]);
            this.#invalidatePool();
    ...
    async pull10(jid, options = {}) {
        ...
        try {
            ({ results, finalTickets } = await this.#transaction(async () => {
                ...
                const myOwnedIds   = new Set(
                    (await this.db.all("SELECT char_id FROM claims WHERE owner_jid = ?", [jid])).map(r => r.char_id)
                );
        ...
        const imageResults = await Promise.all(
            results.map(r =>
                r.char?.booru_tag
                    ? ImageProvider.getRandomUrl(r.char.booru_tag).catch(err => {
                        this.logger.warn("Fallo al obtener imagen para personaje", { charId: r.char.id, error: err.message });
                        return null;
                    })
                    : Promise.resolve(null)
            )
        );
        results.forEach((r, i) => { r.imageUrl = imageResults[i]; });
    ...
    async getMarket(limit = 20, offset = 0) {
        ...
        const items = await this.db.all(`
            ...
            LIMIT ? OFFSET ?
        `, [limit, offset]);
        const { total } = await this.db.get("SELECT COUNT(*) as total FROM market");
    ...
    async listCharacters(options = {}) {
        ...
        const { total } = await this.db.get("SELECT COUNT(*) as total FROM characters");
