...
export class ImageProvider {
    static async getRandomUrl(tag) {
        if (!tag) return null;
        const url = `https://yande.re/post.json?tags=${encodeURIComponent(tag)}&limit=20`;
        try {
            const res = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0" } });
            if (!res.ok) return null;
            const posts = await res.json();
            if (!Array.isArray(posts)) return null;
            const withImage = posts.filter(p => p.sample_url || p.file_url);
    ...
    async pull10(jid, options = {}) {
        ...
        try {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                const droppedTicket = Math.random() < 0.02;
            ...
            await this.db.run(
                ...
                [PULL_COST, jackpotTotal, compensationTotal, p, luck, currentTickets, userJid]
            );
