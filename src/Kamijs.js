...
const PULL_COST     = 3000;   // Costo por tirada de 10 (batch)
...
export class Kamijs {
    ...
    async init() {
        ...
        // ── Migración 1: columnas añadidas en versiones antiguas ─────────────
        if (currentVersion < 1) {
            await this.db.run("UPDATE characters SET value = 3000 WHERE value IS NULL");

            // Chequeamos cada columna individualmente para evitar saltear
            // alguna si otra ya existía en una migración parcial.
            ...
            // Elimina columna group_id legacy de claims si aún existe
        ...
        // ── Migración 2: forzar global_limit = 1 ────────────────────────────
        if (currentVersion < 2) {
    ...
    async cleanInactiveUsers() {
        const cutoff = Date.now() - 1_209_600_000; // 14 días
        // FIX: usar BEGIN IMMEDIATE para consistencia con el resto de transacciones
    ...
    async claimStarter(jid, charId, sock) {
        ...
        try {
            ...
            const char = await this.db.get(
                "SELECT * FROM characters WHERE id = ? COLLATE NOCASE OR LOWER(name) = LOWER(?)",
                [charId, charId]
            );
            if (!char) throw new Error("CHARACTER_NOT_FOUND");

            const claims = (await this.db.get("SELECT COUNT(*) as count FROM claims WHERE char_id = ?", [char.id])).count;
            if (char.global_limit && claims >= char.global_limit) throw new Error("OUT_OF_STOCK");
    ...
    async buyFromMarket(jid, marketId, sock) {
        ...
        try {
            ...
            // Verificar que el vendedor sigue siendo dueño
    ...
    async pull10(jid, options = {}) {
        ...
        try {
            ...
            const results = [];
            // p   = pity counter heredado del usuario
            // luck = bono de tasa acumulado por tiradas fallidas
            ...
            // Candidatos: personajes que aún no alcanzaron su límite global de claims
            ...
            for (let i = 0; i < 10; i++) {
                ...
                // Tasa efectiva: sube escalonadamente según el pity acumulado
                const effectiveRate = Math.min(
                    (p >= 80 ? 0.06 : p >= 60 ? 0.04 : p >= 40 ? 0.025 : HIT_RATE_RW) + luck,
                    1
                );

                // Hit por pity (solo si aún no hubo hit en esta sesión) o por azar
                // Nota: tras un hit p se resetea a 0, por lo que el pity no puede
                // volver a dispararse en las tiradas restantes de la misma sesión.
                // El flag `pityFired` evita explícitamente un doble-pity por si acaso.
                const pityFired = p >= PITY_LIMIT_RW;
                if (pityFired || Math.random() < effectiveRate) {
                    luck = 0; p = 0;
                    ...
                    // Jackpot (1 % de probabilidad, toma 5 % del banco)
                ...
                // Ticket de selección (2 % por tirada)
                ...
                results.push({
                    ...
                    droppedTicket,
                    pity: p,
                    luck: Math.round(luck * 10_000) / 10_000
                });
            }

            // Persistir claims nuevos
            ...
            await this.db.run(
                ...
                [PULL_COST, jackpotTotal, p, luck, currentTickets, userJid]
            );
            await this.db.run("COMMIT");

            // Notificación de tickets fuera de la transacción (fire-and-forget)
            if (sock && chatId) {
